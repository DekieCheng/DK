...
namespace FlexFlowSmartDesktopCCharp
{
    public class exetowinform
    {
        ...
        public IntPtr Start(string FileNameStr)
        {
            ...
            try
            {
                m_AppProcess = GetExistedProcess(FileNameStr);
                if (m_AppProcess == null)
                {
        ...
        public Process GetExistedProcess(string szProcessName)
        {
            try
            {
                string fileName = szProcessName.Substring(szProcessName.LastIndexOf(@"\")+1).Replace(".exe","") ;
        ...
        private void EmbedProcess(Process app, Control control)
        {
            ...
            try
            {
                ...
                SetWindowLong(new HandleRef(this, app.MainWindowHandle), GWL_STYLE, WS_EX_MDICHILD);// WS_VISIBLE);
