...
namespace Model
{
    public class LNChilledWaterFlowMeter : LNMeterBase
    {
        /// <summary>
        ///         ''' Entity udtLNChilledWaterFlowMeter
        ///         ''' </summary>
        public LNChilledWaterFlowMeter()
        {
        }

        public LNChilledWaterFlowMeter(int id)
        {
            try
            {
                DataTable dt = LNGlobal.g_DB.GetTable(" udtLNChilledWaterFlowMeter with(nolock) ", string.Format("Where ID = {0} Order By ID", id.ToString()));
                if (dt.Rows.Count > 0)
                {
                ...
                {
                    throw new Exception(string.Format("Cannot find the record from udtLNChilledWaterFlowMeter by using the ID[{0}] value.", id.ToString()));
                }
            }
            catch (Exception ex)
            { throw ex; }
