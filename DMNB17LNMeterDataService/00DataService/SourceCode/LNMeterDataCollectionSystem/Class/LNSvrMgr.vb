...
Namespace ServiceMgr
    Public Class LNSvrMgr
        Public Shared Function ToGenCommand(ByVal meterAddr As String, ByVal orgCommand As String) As String
            Dim cmdByte As String() = New String(5) {}
            Dim splittedCommand As String() = orgCommand.Split(New Char() {" "c})
            cmdByte(0) = Conversion.Hex(meterAddr).PadLeft(2, "0") '(Int32.Parse(meterAddr, NumberStyles.HexNumber)).ToString().PadLeft(2, "0") '(meterAddr).PadLeft(2, "0"c)
        ...
        Private Shared Function ProcessToParserReceivedData(ByVal lNMeterCategory As LNMeterCategory, ByVal orgReceivedDecData As Hashtable) As Hashtable
            Try
        ...
        Private Shared Sub ParallelExecutionforElectricityMeter(ByVal lNMeterCollection As LNMeterCollection, ByVal currMeterCategory As LNMeterCategory)
            ...
            Try
                LNLogMgr.WriteLogAsy("ParallelExecution started")
                ...
                Next
                stopwatch.[Stop]()
                LNLogMgr.WriteLogAsy("ParallelExecution time : " & stopwatch.ElapsedMilliseconds & vbCrLf)
            Catch ex As Exception
                lNRuntimeArguments.RuntimeException = ex
                LNLogMgr.WriteLogAsy(ex)
            Finally
        ...
        Private Shared Sub ParallelExecutionforWaterMeter(ByVal lNMeterCollection As LNMeterCollection, ByVal currMeterCategory As LNMeterCategory)
            ...
            Parallel.ForEach(CType(lNMeterCollection, IEnumerable(Of LNWaterMeter)), Sub(currentMeter)
                                                                                         ...
                                                                                         Thread.Sleep(10)
                                                                                     End Sub)
            LNLogMgr.WriteLogAsy("ParallelExecution for ParallelExecutionforWaterMeter: " & stopwatch.ElapsedMilliseconds & vbCrLf)
            stopwatch.[Stop]()
        End Sub

        Private Const KEYOFORGDATA As String = "KEYOFORGDATA"
        Private Const NEWPROTOCAL As String = "NEWPROTOCAL"
        Private Shared Sub ToReadDatafromHost(ByVal lNMeterCategory As LNMeterCategory, ByVal lNElectricityMeter As LNElectricityMeter)
            ...
            Try
                ...
                Dim lstReceivedData As Hashtable = New Hashtable

                For Each cmd As String In lNMeterCategory.Command.Split("|")
                    Dim outputCmd As String
                    Try
                        Dim newcmd As String = ToAutoIdentifyCommand(lNElectricityMeter.IsNewProtocal, cmd)
                        If String.IsNullOrEmpty(newcmd) Then Continue For
                        outputCmd = ToGenCommand(lNElectricityMeter.MeterAddr, newcmd)
                        LNLogMgr.WriteLogAsy(String.Format("Try to read data from host [{0}:{1}] by the command [{2}].",
                                                                lNSerialPortServer.HostIP, lNSerialPortServer.HostPort, outputCmd))
                        If String.IsNullOrEmpty(outputCmd) Then Continue For
                    Catch ex As Exception
                        Throw ex
                    End Try

                    'Using _AsyncTcpClient As AsyncTcpClient = New AsyncTcpClient(lNSerialPortServer.HostIP, lNSerialPortServer.HostPort, outputCmd)
                    ...
                    '    If (Not lstReceivedData.ContainsKey(KEYOFORGDATA)) Then
                    '        lstReceivedData.Add(KEYOFORGDATA, ReceivedDataHex)
                    '    End If
            ...
            Catch ex As Exception
                LNLogMgr.WriteLogAsy(String.Format("Exception Error Message:{0} for remote host [{1}:{2}] by command [{3}]", ex.Message, lNSerialPortServer.HostIP, lNSerialPortServer.HostPort, lNMeterCategory.Command))
                lNRuntimeArguments.RuntimeException = ex
                LNLogMgr.WriteLogAsy(ex)
            Finally
                '_AsyncTcpClient = Nothing
                ProcessToSaveRuntimeErrLog(lNRuntimeArguments)
            End Try
        End Sub

        Private Shared Function ToAutoIdentifyCommand(ByVal IsNewProtocal As Boolean, ByVal OrgCmd As String) As String
            If OrgCmd.ToUpper.Contains("{NEW}") Then
                If Not IsNewProtocal Then
                    Return String.Empty
                Else
                    Return OrgCmd.ToUpper.Replace("{NEW}", "")
                End If
            Else
                If IsNewProtocal Then
                    Return String.Empty
                Else
                    Return OrgCmd
                End If
