        ...
        public bool FireMeterClient()
        {
            ...
            using (Socket client = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp))
            {
                try
                {
                    ...
                    if (client.Connected)
                    {
                        byte[] outCmd = LNGlobal.HexStringToBytes(m_Command);
                        ProcessToSend(client, outCmd);
                        sendDone.WaitOne(300);

                        ProcessToReceive(client);
                        receiveDone.WaitOne(300);
                    }
        ...
        private void ProcessToReceive(Socket client)
        {
            try
            {
                if (client.Connected)
                {
                    ...
                    client.ReceiveTimeout = 120 * 1000;
        ...
        private void ReceiveCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    LNLogMgr.WriteLogAsy(string.Format("Received data length:{0}, KWh(int):{1}, KWh(0x):{2}, Data(string): [{3}], Data(Hex):{4} from host [{5}:{6}] by command [{7}]",
                        bytesRead.ToString(), int_KWh.ToString(), KWh, data, recData, m_HostServerIP, m_HostServerPort.ToString(), m_Command));
