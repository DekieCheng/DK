...
namespace LNMeterInfrastructure.Common
{
    public static class LNGlobal
    {
        ...
        //    {
        //        string str2 = "";
        //        byte[] buffer4 = new byte[] { 0xBA, 0xED, 0xE8, 0x16, 0x31, 0x79, 0xC3, 0x8D, 0x3E, 0xE3, 0x6F, 0x1, 0xC5, 0x29, 0x31, 0x90, 0x35, 0xC0, 110, 160, 15, 5, 0xC4, 0x5F, 0x83, 0x3A, 0x42, 0xEA, 0x8B, 3, 0x1D, 0x9D };
        //        managed.IV = new byte[] { 0xD9, 90, 0xE7, 80, 0x88, 0xB0, 7, 90, 0x23, 0x59, 0x9, 20, 0x11, 0x41, 0x5B, 0x49 };
        //        managed.Key = buffer4;
        //        byte[] buffer2 = Convert.FromBase64String(inputString);
        //        byte[] buffer = new byte[(buffer2.Length + 1) - 1 + 1];
        //        MemoryStream stream2 = new MemoryStream(buffer2);
        //        CryptoStream stream = new CryptoStream(stream2, managed.CreateDecryptor(), CryptoStreamMode.Read);
        //        stream.Read(buffer, 0, buffer.Length);

        //        foreach (var ch in encoding.GetChars(buffer))
        //        {
        //            if ((Strings.Asc(ch) == 0))
        //                break;
        ...
        public static string CommonParsePassword(string inputstring, bool IfDecrypt)
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= str1.GetLength(0) - 1; i++)
                {
                    ...
                    {
                        if (IfDecrypt)
                            inputstring = inputstring + ";" + str1[i].Split(splitter)[0] + "=" + DecryptString(str1[i].Split(splitter)[1] + "==");
                        else
                            inputstring = inputstring + ";" + str1[i].Split(splitter)[0] + "=" + EncryptString(str1[i].Split(splitter)[1] + "==");
                    }
                    else
                        inputstring = inputstring + ";" + str1[i];
                }
