...
namespace Model
{
    public class LNSerialPortServer
    {
        ...
        public LNSerialPortServer(int id)
        {
            try
            {
                DataTable dt = LNGlobal.g_DB.GetTable(" udtLNSerialPortServer  with(nolock) ", string.Format("Where ID = {0} Order By ID", id.ToString()));
                if (dt.Rows.Count > 0)
                {
        ...
        private LNMeterCollection _LNElectricityMeterCollection = null;
        private LNMeterCollection _LNWaterMeterCollection = null;

        public LNMeterCollection LNElectricityMeterCollection
        {
            set
            {
                _LNElectricityMeterCollection = value;
            }
            get
            {
                if (_id < 0) return null;
                if (_LNElectricityMeterCollection == null)
                {
                    _LNElectricityMeterCollection = new LNMeterCollection("udtLNElectricityMeter", this.ID);
                }

                return _LNElectricityMeterCollection;
            }
        }

        public LNMeterCollection LNWaterMeterCollection
        {
            set
            {
                _LNWaterMeterCollection = value;
            }
            get
            {
                if (_id < 0) return null;
                if (_LNWaterMeterCollection == null)
                {
                    _LNWaterMeterCollection = new LNMeterCollection("udtLNWaterMeter", this.ID);
                }

                return _LNWaterMeterCollection;
            }
        }
    }
}
