...
{
    public class CommonSql: IDisposable
    {
        ...
        /// 从配置文件中读取连接字符串
        /// </summary>
        public CommonSql(string constr)
        {
            if (System.Configuration.ConfigurationManager.ConnectionStrings[constr] != null)
            {
                _connectionString = Encry.Encry.Decryptosdp(System.Configuration.ConfigurationManager.ConnectionStrings[constr].ConnectionString);
            }
            else
            {
                throw new Exception("The Connection String can not be found!");
            }
        }
